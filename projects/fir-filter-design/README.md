...
## 문제 해결 과정
FIR 연산이 진행되는 도중 계수를 바꾸면 tap마다 서로 다른 계수가 섞여서 waveform이 고르게 안 나오는 문제가 있었습니다. 계수 업데이트 구간과 필터 연산 구간을 완전히 분리된 타이밍으로 설계해서 해결했습니다 — 필터 연산 단계에서는 SRAM을 Read-only로, 계수 업데이트 단계에서는 Write-only로 동작하도록 FSM에서 통제했습니다.

## 결과
Impulse Response Waveform 검증 결과, 계수가 정상적으로 로딩되고 Accumulator와 enable 신호 타이밍이 정상 동작하며 FIR 구조에 오류가 없음을 확인했습니다. 출력 파형은 Kaiser window 기반 계수의 대칭 구조(Linear Phase FIR)를 그대로 보여주었습니다.
